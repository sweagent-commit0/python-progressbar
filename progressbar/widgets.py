...
class FormatWidgetMixin(abc.ABC):
    ...
    def __call__(self, progress: ProgressBarMixinBase, data: Data, format: types.Optional[str]=None) -> str:
        ...
        try:
            ...
            logger.exception('Error while formatting %r with data: %r', format_, data)
            raise
...
class WidgetBase(WidthWidgetMixin, metaclass=abc.ABCMeta):
    ...
    def __call__(self, progress: ProgressBarMixinBase, data: Data) -> str:
        ...
        """
    _fixed_colors: ClassVar[TFixedColors] = TFixedColors(fg_none=None, bg_none=None)
...
class AutoWidthWidgetBase(WidgetBase, metaclass=abc.ABCMeta):
    ...
    @abc.abstractmethod
    def __call__(self, progress: ProgressBarMixinBase, data: Data, width: int=0) -> str:
...
class FormatLabel(FormatWidgetMixin, WidgetBase):
    ...
    """
    mapping: ClassVar[types.Dict[str, types.Tuple[str, types.Any]]] = dict(finished=('end_time', None), last_update=('last_update_time', None), max=('max_value', None), seconds=('seconds_elapsed', None), start=('start_time', None), elapsed=('total_seconds_elapsed', utils.format_time), value=('value', None))

    def __init__(self, format: str, **kwargs):
        FormatWidgetMixin.__init__(self, format=format, **kwargs)
        WidgetBase.__init__(self, **kwargs)

    def __call__(self, progress: ProgressBarMixinBase, data: Data, format: types.Optional[str]=None):
...
class SamplesMixin(TimeSensitiveWidgetBase, metaclass=abc.ABCMeta):
    ...
    def __call__(self, progress: ProgressBarMixinBase, data: Data, delta: bool=False):
        ...
        if progress.last_update_time - sample_time > self.INTERVAL:
            ...
            if isinstance(self.samples, datetime.timedelta):
                ...
                while sample_times[2:] and minimum_time > sample_times[1] and (minimum_value > sample_values[1]):
                    sample_times.pop(0)
                    sample_values.pop(0)
...
class ETA(Timer):
    ...
    def __init__(self, format_not_started='ETA:  --:--:--', format_finished='Time: %(elapsed)8s', format='ETA:  %(eta)8s', format_zero='ETA:  00:00:00', format_na='ETA:      N/A', **kwargs):
...
class AbsoluteETA(ETA):
    ...
    def __init__(self, format_not_started='Estimated finish time:  ----/--/-- --:--:--', format_finished='Finished at: %(elapsed)s', format='Estimated finish time: %(eta)s', **kwargs):
        ETA.__init__(self, format_not_started=format_not_started, format_finished=format_finished, format=format, **kwargs)
...
class AdaptiveETA(ETA, SamplesMixin):
    ...
    def __init__(self, exponential_smoothing=True, exponential_smoothing_factor=0.1, **kwargs):
...
class SmoothingETA(ETA):
    ...
    def __init__(self, smoothing_algorithm: type[algorithms.SmoothingAlgorithm]=algorithms.ExponentialMovingAverage, smoothing_parameters: dict[str, float] | None=None, **kwargs):
        ...
        self.smoothing_algorithm = smoothing_algorithm(**self.smoothing_parameters or {})
        ETA.__init__(self, **kwargs)
...
class DataSize(FormatWidgetMixin, WidgetBase):
    ...
    def __init__(self, variable='value', format='%(scaled)5.1f %(prefix)s%(unit)s', unit='B', prefixes=('', 'Ki', 'Mi', 'Gi', 'Ti', 'Pi', 'Ei', 'Zi', 'Yi'), **kwargs):
...
class FileTransferSpeed(FormatWidgetMixin, TimeSensitiveWidgetBase):
    ...
    def __init__(self, format='%(scaled)5.1f %(prefix)s%(unit)-s/s', inverse_format='%(scaled)5.1f s/%(prefix)s%(unit)-s', unit='B', prefixes=('', 'Ki', 'Mi', 'Gi', 'Ti', 'Pi', 'Ei', 'Zi', 'Yi'), **kwargs):
        self.unit = unit
        self.prefixes = prefixes
    ...
    def __call__(self, progress: ProgressBarMixinBase, data, value=None, total_seconds_elapsed=None):
        """Updates the widget with the current SI prefixed speed."""
        if value is None:
            value = data['value']
        elapsed = utils.deltas_to_seconds(total_seconds_elapsed, data['total_seconds_elapsed'])
        if value is not None and elapsed is not None and (elapsed > 2e-06) and (value > 2e-06):
...
class ColoredMixin:
    _fixed_colors: ClassVar[TFixedColors] = TFixedColors(fg_none=colors.yellow, bg_none=None)
    _gradient_colors: ClassVar[TGradientColors] = TGradientColors(fg=colors.gradient, bg=None)
...
class SimpleProgress(FormatWidgetMixin, ColoredMixin, WidgetBase):
    ...
    max_width_cache: dict[types.Union[str, tuple[float, float | types.Type[base.UnknownLength]]], types.Optional[int]]
    ...
    def __call__(self, progress: ProgressBarMixinBase, data: Data, format=None):
        ...
        max_width: types.Optional[int] = self.max_width_cache.get(key, self.max_width)
        if not max_width:
            ...
            for value in key:
                ...
                if (width := progress.custom_len(FormatWidgetMixin.__call__(self, progress, temporary_data, format=format))):
...
class Bar(AutoWidthWidgetBase):
    ...
    def __init__(self, marker='#', left='|', right='|', fill=' ', fill_left=True, marker_wrap=None, **kwargs):
...
class ReverseBar(Bar):
    ...
    def __init__(self, marker='#', left='|', right='|', fill=' ', fill_left=False, **kwargs):
        """Creates a customizable progress bar.
        ...
        fill_left - whether to fill from the left or the right
        """
        Bar.__init__(self, marker=marker, left=left, right=right, fill=fill, fill_left=fill_left, **kwargs)
...
class FormatCustomText(FormatWidgetMixin, WidgetBase):
    ...
    def __init__(self, format: str, mapping: types.Optional[types.Dict[str, types.Any]]=None, **kwargs):
        self.format = format
    ...
    def __call__(self, progress: ProgressBarMixinBase, data: Data, format: types.Optional[str]=None):
        return FormatWidgetMixin.__call__(self, progress, self.mapping, format or self.format)
...
class GranularBar(AutoWidthWidgetBase):
    ...
    def __init__(self, markers=GranularMarkers.smooth, left='|', right='|', **kwargs):
        """Creates a customizable progress bar.
...
class FormatLabelBar(FormatLabel, Bar):
    ...
    def __call__(self, progress: ProgressBarMixinBase, data: Data, width: int=0, format: FormatString=None):
        ...
        return self._apply_colors(bar[:center_left], data) + self._apply_colors(center, data) + self._apply_colors(bar[center_right:], data)
...
class CurrentTime(FormatWidgetMixin, TimeSensitiveWidgetBase):
    ...
    def __init__(self, format='Current Time: %(current_time)s', microseconds=False, **kwargs):
...
class JobStatusBar(Bar, VariableMixin):
    ...
    def __init__(self, name: str, left='|', right='|', fill=' ', fill_left=True, success_fg_color=colors.green, success_bg_color=None, success_marker='█', failure_fg_color=colors.red, failure_bg_color=None, failure_marker='X', **kwargs):
        VariableMixin.__init__(self, name)
        self.name = name
        self.job_markers = []
        self.left = string_or_lambda(left)
        ...
        Bar.__init__(self, left=left, right=right, fill=fill, fill_left=fill_left, **kwargs)
